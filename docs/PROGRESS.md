...
- 实现进度：S1 进行中；学习者已完成消息列表、append 和 input，已运行验证。能解释覆盖会丢掉历史会话。尚未调用模型，S2 提前安排终端交互练习，未验收；S3–S8 未开始。
- 下一步：学习者在 hello_agent.py 实现连续输入、空白过滤、/history、/clear、/exit；完成后再接模型。
...
1. 学习者实现终端会话循环：初始仅有 system 消息；普通输入追加 user；空白跳过；/history 显示历史；/clear 仅保留 system；/exit 退出。命令不进入消息历史。
2. 验收序列：输入两条普通消息后历史有三条；空白和命令不增加条数；清空后仅剩 system；退出后无新的消息。
